...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        fronts: List[int] = convert_params(data[i * one_test_number + 0], 'List[int]')
        backs: List[int] = convert_params(data[i * one_test_number + 1], 'List[int]')
        real_res: int = convert_params(data[i * one_test_number + 2], 'int')

        my_res: int = sol.flipgame(fronts, backs)
