...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        nums1: List[int] = convert_params(data[i * one_test_number + 0], 'List[int]')
        nums2: List[int] = convert_params(data[i * one_test_number + 1], 'List[int]')
        real_res: float = convert_params(data[i * one_test_number + 2], 'float')

        my_res: float = sol.findMedianSortedArrays(nums1, nums2)

        check_result = compare_result(f"{i + 1}", my_res, real_res, 'float')
        if not check_result:
            fail_test += 1
