...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int w;
        convert_params(data[i * one_test_number + 1], w);
        vector<int> profits;
        convert_params(data[i * one_test_number + 2], profits);
        vector<int> capital;
        convert_params(data[i * one_test_number + 3], capital);
        int real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        int my_res = sol.findMaximizedCapital(k, w, profits, capital);
