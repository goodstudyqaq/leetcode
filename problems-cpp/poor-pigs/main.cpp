...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int buckets;
        convert_params(data[i * one_test_number + 0], buckets);
        int minutesToDie;
        convert_params(data[i * one_test_number + 1], minutesToDie);
        int minutesToTest;
        convert_params(data[i * one_test_number + 2], minutesToTest);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.poorPigs(buckets, minutesToDie, minutesToTest);
