...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        vector<string> words;
        convert_params(input_param[0], words);
        WordFilter sol = WordFilter(words);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "f") {
                string pref;
                convert_params(input_param[0], pref);
                string suff;
                convert_params(input_param[1], suff);
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.f(pref, suff);
