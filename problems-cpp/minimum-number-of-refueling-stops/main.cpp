...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int startFuel;
        convert_params(data[i * one_test_number + 1], startFuel);
        vector<vector<int>> stations;
        convert_params(data[i * one_test_number + 2], stations);
        int real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        int my_res = sol.minRefuelStops(target, startFuel, stations);
