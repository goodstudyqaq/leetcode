...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        MyCalendarThree sol = MyCalendarThree();

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "book") {
                int startTime;
                convert_params(input_param[0], startTime);
                int endTime;
                convert_params(input_param[1], endTime);
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.book(startTime, endTime);
