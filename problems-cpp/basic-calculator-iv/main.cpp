...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        convert_params(data[i * one_test_number + 1], evalvars);
        vector<int> evalints;
        convert_params(data[i * one_test_number + 2], evalints);
        vector<string> real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        vector<string> my_res = sol.basicCalculatorIV(expression, evalvars, evalints);
