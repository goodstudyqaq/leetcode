...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int sx;
        convert_params(data[i * one_test_number + 0], sx);
        int sy;
        convert_params(data[i * one_test_number + 1], sy);
        int tx;
        convert_params(data[i * one_test_number + 2], tx);
        int ty;
        convert_params(data[i * one_test_number + 3], ty);
        bool real_res;
        convert_params(data[i * one_test_number + 4], real_res);

        bool my_res = sol.reachingPoints(sx, sy, tx, ty);
