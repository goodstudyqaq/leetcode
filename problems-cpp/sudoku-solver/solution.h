            ...
            for (int j = 0; j < 9; j++) {
                ...
                if (it == '.') {
                    for (int k = 1; k <= 9; k++) {
                        int now = idx * 9 + k;
                        dlx.link(now, idx + 1);
                        dlx.link(now, 81 + (row - 1) * 9 + k);
                        dlx.link(now, 81 * 2 + (col - 1) * 9 + k);
                        dlx.link(now, 81 * 3 + (grid - 1) * 9 + k);
                    }
                } else {
                    int temp = it - '0';
                    int now = idx * 9 + temp;
                    dlx.link(now, idx + 1);
                    dlx.link(now, 81 + (row - 1) * 9 + temp);
                    dlx.link(now, 81 * 2 + (col - 1) * 9 + temp);
                    dlx.link(now, 81 * 3 + (grid - 1) * 9 + temp);
                }
            }
        }
        debug("link");
        bool tmp = dlx.exact_dance(0);
        debug(tmp);
        int dep = dlx.ansd;
        debug(dep);
        for (int i = 0; i < dep; i++) {
