...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int indexDiff;
        convert_params(data[i * one_test_number + 1], indexDiff);
        int valueDiff;
        convert_params(data[i * one_test_number + 2], valueDiff);
        bool real_res;
        convert_params(data[i * one_test_number + 3], real_res);

        bool my_res = sol.containsNearbyAlmostDuplicate(nums, indexDiff, valueDiff);
