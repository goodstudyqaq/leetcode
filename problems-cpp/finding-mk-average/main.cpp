...
int main() {
    ...
    for (int i = 0; i < test_number; i++) {
        ...
        int m;
        convert_params(input_param[0], m);
        int k;
        convert_params(input_param[1], k);
        MKAverage sol = MKAverage(m, k);

        for (int j = 1; j < func_num; j++) {
            input_param = split_str_to_params(input_params[j]);
            if (function_names[j] == "addElement") {
                int num;
                convert_params(input_param[0], num);
                sol.addElement(num);
            }
            if (function_names[j] == "calculateMKAverage") {
                int real_res;
                convert_params(output_params[j], real_res);
                int my_res = sol.calculateMKAverage();
